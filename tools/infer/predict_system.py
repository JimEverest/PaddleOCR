...
def argparse_2_object(input_args):
    """
    Convert different types of input arguments to an object with named attributes.
    """
    if isinstance(input_args, dict):
        # Convert dictionary to SimpleNamespace object
        return SimpleNamespace(**input_args)
    elif isinstance(input_args, argparse.Namespace):
        # Already a Namespace object
        return input_args
...
def run(**kwargs):
    # 将参数映射为Namespace对象。
    # Solution 1: 使用argparse.ArgumentParser().parse_args()函数
    args = create_args([])

    # Convert args to a general object
    args = argparse_2_object(args)

    # Update args with additional kwargs
    for key, value in kwargs.items():
        if hasattr(args, key):
            setattr(args, key, value)
        else:
            print("Warning: args has no attribute {}".format(key), "...Skiping it")


    main(args)

    # Solution 2: 使用SimpleNamespace函数
    # from types import SimpleNamespace
    # d = {
